...
export function registerHouseholdRoutes(app: OpenAPIHono<AuthedEnv>, db: HouseholdDb): void {
  ...
  app.openapi(getRoute, async (c) => {
    // Device-readable: a dashboard is allowed to know its own household's
    // name, and requireMembership() already confines it to that one.
  ...
  app.openapi(patchRoute, async (c) => {
    // Renaming/deleting the household is not in a device's allowed action
    // set (FEATURE_ANALYSIS.md's device authorization table) even though
    // the rename itself needs no particular user's identity below.
    requireUser(c);
    const { hid } = c.req.valid('param');
    const body = c.req.valid('json');
    try {
